...
public class AlarmRingtoneDialog extends DialogFragment implements
    ...
    public void onClick(DialogInterface dialog, int which) {
        if (which == DialogInterface.BUTTON_POSITIVE) {
            if (mPreAlarm){
                mAlarm.preAlarmAlert = mRingtone;
                mAlarm.preAlarmVolume = mVolume;
                if (mEnabledCheckbox.isChecked()) {
                    mAlarm.preAlarmVolume = -1; 
                } else {
                    mAlarm.preAlarmVolume = mMaxVolumeSeekBar.getProgress() + 1;
                }
            } else {
                if (mMediaStart){
                    mAlarm.alert = Alarm.NO_RINGTONE_URI;
                    mAlarm.mediaStart = true;
                    mAlarm.alarmVolume = -1;
                } else {
                    mAlarm.alert = mRingtone;
                    mAlarm.mediaStart = false;
                    mAlarm.alarmVolume = mVolume;
                    if (mEnabledCheckbox.isChecked()) {
                        mAlarm.alarmVolume = -1; 
                    } else {
                        mAlarm.alarmVolume = mMaxVolumeSeekBar.getProgress() + 1;
                    }
                }
            }
            mAlarm.increasingVolume = mIncreasingVolumeValue;
